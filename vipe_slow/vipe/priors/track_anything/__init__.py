...
class TrackAnythingPipeline:
    def __init__(
        ...
        if not aot_ckpt_path.exists():
            ...
            # NOTE (speed_up_video_evaluation): the original call here passed a
            # Drive *view URL* plus `fuzzy=True`. Modern gdown removed the `fuzzy`
            # kwarg, so this old checkout crashed on first run with
            # `TypeError: download() got an unexpected keyword argument 'fuzzy'`.
            # Upstream ViPE v1.2.0 fixed it the same way: pass the file id directly.
            # This is a dependency-drift bug, not a performance one -- recorded
            # because "the old version is slower" is a claim you can only make
            # after the old version runs at all.
            gdown.download(
