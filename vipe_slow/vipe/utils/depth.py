...
def normal_weight_from_xyz(xyz: torch.Tensor, robust: bool = True) -> torch.Tensor:
    """
    Compute per-pixel surface normal (camera-1 space) and a confidence weight.
    Returns [..., H, W, 4] where last channel is weight in [0,1].
    Tries the compiled extension; falls back to a PyTorch implementation.
    """
    batch_dim = (xyz.dim() == 4)  # [B,H,W,3] or [H,W,3]
    if not batch_dim:
        xyz = xyz.unsqueeze(0)

    # Fast path: compiled extension if available
    try:
        from .ext import _C  # compiled module
        out = _C.normal_weight_from_xyz(xyz.contiguous())  # -> [B,H,W,4]
        if not batch_dim:
            out = out.squeeze(0)
        return out
    except Exception:
        pass  # fall back to PyTorch below

    # ---------- PyTorch fallback (shape-safe, no F.pad) ----------
    # xyz: [B,H,W,3]
    B, H, W, _ = xyz.shape

    # Forward differences with edge replication to keep shape [B,H,W,3]
    dx = xyz[:, :, 1:, :] - xyz[:, :, :-1, :]          # [B,H,W-1,3]
    dx = torch.cat([dx[:, :, :1, :], dx], dim=2)       # replicate first column -> [B,H,W,3]

    dy = xyz[:, 1:, :, :] - xyz[:, :-1, :, :]          # [B-? no, B,H-1,W,3]
    dy = torch.cat([dy[:, :1, :, :], dy], dim=1)       # replicate first row   -> [B,H,W,3]

    # Normals (make sure to specify dim)
    n = torch.linalg.cross(dx, dy, dim=-1)             # [B,H,W,3]
    n = n / (torch.linalg.norm(n, dim=-1, keepdim=True) + 1e-6)

    # View direction (from surface point to camera at origin)
    v = -xyz / (torch.linalg.norm(xyz, dim=-1, keepdim=True) + 1e-6)  # [B,H,W,3]

    # Flip back-facing normals to face camera; weight = |cos(theta)|
    cos_nv = (n * v).sum(dim=-1, keepdim=True)         # [B,H,W,1]
    n = torch.where(cos_nv < 0, -n, n)
    w = cos_nv.abs().clamp(0, 1)                       # [B,H,W,1]

    # Optional robust smoothing for stability
    if robust:
        w = torch.nn.functional.avg_pool2d(
            w.permute(0, 3, 1, 2), kernel_size=3, stride=1, padding=1
        ).permute(0, 2, 3, 1)

    out = torch.cat([n, w], dim=-1)                    # [B,H,W,4]
    if not batch_dim:
        out = out.squeeze(0)
    return out
...
def _zbuffer_nearest_mask(
    uv2: torch.Tensor,          # (B,H,W,2) in pixel coords
    depth2: torch.Tensor,       # (B,H,W)
    ...
    """
    For each target (novel-view) pixel, keep only the source with the smallest depth2.
    Returns a (B,H,W) bool mask to multiply into flow weights.

    Notes:
    - Uses integer binning consistent with bilinear_splatting: floor(uv + 0.5).
    - Requires PyTorch with scatter_reduce_ (2.0+).
    """
    B, H, W, _ = uv2.shape
    x = torch.floor(uv2[..., 0] + 0.5).clamp(0, tgt_w - 1).long()
    y = torch.floor(uv2[..., 1] + 0.5).clamp(0, tgt_h - 1).long()

    keep_mask = torch.zeros_like(valid_mask, dtype=torch.bool)

    # Per-batch to keep memory in check (and avoid mixing indices across batches)
    for b in range(B):
        vm = valid_mask[b]  # (H,W)
        if not vm.any():
            continue
        ...
        xb = x[b][vm]               # (N,)
        yb = y[b][vm]               # (N,)
        zb = depth2[b][vm].contiguous().view(-1)  # (N,)
        idx = (yb * tgt_w + xb).view(-1)          # (N,)

        # z-buffer: per target pixel minimum depth
        zbuf = torch.full((tgt_h * tgt_w,), float('inf'),
                          device=zb.device, dtype=zb.dtype)
        # amin over collisions
        zbuf.scatter_reduce_(0, idx, zb, reduce="amin", include_self=True)

        # Mark only nearest contributors (ties allowed within eps)
        nearest = zb <= zbuf.index_select(0, idx) + eps  # (N,)

        # Write back to (H,W)
        temp = torch.zeros_like(vm, dtype=torch.bool)
        temp[vm] = nearest
        keep_mask[b] = temp
...
from vipe.utils.visualization import *
...
def reproject_fixed(
    ...
    camera_model = camera_type.build_camera_model(intrinsic1).scaled(1.0 / subsample_factor)

    from einops import rearrange
    xyz2 = xyz2.squeeze(0)
    xyz2 = rearrange(xyz2, 'h w c -> (h w) c')
    order = torch.argsort(xyz2[:, 2], descending=False)
    xyz2 = xyz2[order]

    frame1 = frame1.squeeze(0)
    frame1 = rearrange(frame1, 'h w c -> (h w) c')
    frame1 = frame1[order]

    xyz_h = torch.cat([xyz2, torch.ones(xyz2.shape[0], 1)], dim=1)
