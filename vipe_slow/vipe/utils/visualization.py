...
def draw_points_batch(
    ...
    eps = 1e-9
    inc = np.ones_like(color)
    cnt = np.zeros_like(canvas) + eps
    ...
    for rel_pos in stencil:
        ...
        if isinstance(color, np.ndarray):
            canvas_f[pos[:, 1], pos[:, 0]] += (color[in_bound])
            #canvas[pos[:, 1], pos[:, 0]] = color[in_bound]
        else:
            canvas_f[pos[:, 1], pos[:, 0]] = color or (0, 255, 0)
    
    return np.clip(np.round(canvas_f/cnt), 0, 255).astype(canvas.dtype)
