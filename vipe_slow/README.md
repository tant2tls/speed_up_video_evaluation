<!--
================================================================================
speed_up_video_evaluation NOTE — read this before the upstream README below.
================================================================================

`vipe_slow/` is a vendored, deliberately-OLD checkout of NVIDIA ViPE, kept as the
"before" side of this repo's benchmark. The parent README.md and report.md are the
real documentation; this file is upstream's, preserved for reference.

What was changed in this checkout, and why:

  * `run_infer.py`  — a sharded driver that hoists `make_pipeline()` OUT of the
                      per-video loop (the original `run.py`, kept beside it,
                      rebuilds it per video and folds model-load into video #1).
  * `run_fork.sh`   — bash-fork launcher, one process per GPU, threads pinned to
                      (cgroup quota / G). Pass THREADS=0 to reproduce the unpinned
                      thread-storm.
  * `run_ray.py`    — Ray actor pool + dynamic work queue.
  * `run_ray_author.py` — the ViPE author's OWN Ray design (tasks, one per video),
                      reproduced from nv-tlabs/vipe issue #41 and measured.
  * `summarize.py`  — per-shard timing roll-up (CPU-s / GPU-busy-s per video).

Two real bugs were fixed to make this old checkout run at all (both dependency
drift, both fixed the same way upstream v1.2.0 did):
  * `gdown.download(..., fuzzy=True)` -> `id=...` in
    vipe/priors/track_anything/__init__.py and vipe/slam/networks/droid_net.py.

Everything unrelated (demo videos, prior outputs, GUI, multinode scripts, 1.8 GB)
was moved to `_archive/`, not deleted.

The `cwd`-first `sys.path` rule means running from this directory imports THIS
checkout's `vipe/`, not the editable `nvidia-vipe` install in the env — which is
how a slow and a fast ViPE coexist on one box.
================================================================================
-->
